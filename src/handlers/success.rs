...
pub fn handle_success(success_type: SuccessType) {
    ...
    let message = match success_type {
        ...
        SuccessType::TokenVerified { path, url, address } => {
            // Determine the source: prioritize path > url > address
            let source = path.or(url).or(address.clone()).unwrap_or("".to_string());

            let mut message = format!("Verified successfully from: {}", source);

            // Only add the note if the address is None
            if address.is_none() {
